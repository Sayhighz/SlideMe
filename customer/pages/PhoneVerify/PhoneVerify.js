...
function PhoneVerify({ onLogin }) {
    ...
    const handleOtpChange = (index, value) => {
        ...
        if (value) {
            // ถ้ามีค่าที่กรอกและไม่ใช่ช่องสุดท้าย ให้ไปยังช่องถัดไป
            if (index < otpRefs.current.length - 1) {
                otpRefs.current[index + 1].current.focus();
            }
        } else {
            // ถ้าค่าที่กรอกเป็นค่าว่างและไม่ใช่ช่องแรก ให้เลื่อนไปยังช่องก่อนหน้า
            if (index > 0) {
                otpRefs.current[index - 1].current.focus();
            }
        }
    ...
    return (
        ...
        <SafeAreaView style={tw`flex-1 bg-white`}>
            ...
            <View style={tw`flex-2 items-center p-5`}>
                ...
                <TouchableOpacity
                    style={tw`bg-[#60B876] rounded-lg mt-8 w-3/4 p-3`}
