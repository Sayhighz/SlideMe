...
const DriverCard = React.memo(({ item, selectedDriver, onDriverSelect, fee }) => {
  // Animation for card selection
  const [animation] = React.useState(new Animated.Value(selectedDriver?.id === item.id ? 1 : 0));
    ...
    >
      ...
      >
        <View style={tw`flex-row items-center`}>
          ...
          <View style={tw`mr-3`}>
            <View style={tw`h-12 w-12 rounded-full bg-blue-50 items-center justify-center`}>
              <MaterialIcons 
                name="person" 
                size={28} 
...
const DriverList = ({
  ...
  const renderEmptyList = () => (
    <View style={tw`flex-1 justify-center items-center py-10`}>
      ...
      <Text style={[styles.globalText, tw`text-sm text-gray-400 mt-1 text-center px-10`]}>
  ...
  if (isLoading) {
    return (
      <View style={tw`flex-1 justify-center items-center py-6`}>
        <ProgressBar status="pending" />
        <Text style={[styles.globalText, tw`text-gray-500 mt-4`]}>
          กำลังค้นหาคนขับ...
        </Text>
