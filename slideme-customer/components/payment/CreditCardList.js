...
const CreditCardList = ({
  ...
  const renderCard = ({ item }) => {
    ...
    return (
      <Pressable
        ...
        android_ripple={{
          color: isSelected ? "#88D49A" : "#f0f0f0",
          borderless: false,
          radius: 120,
        }}
      >
        <View
          style={tw`w-14 h-14 items-center justify-center bg-white rounded-xl ${
  ...
  const sectionData =
    paymentMethods.length > 0
      ? [
          {
            title: "บัตรของฉัน",
            data: paymentMethods.filter((item) => item.is_active),
          },
        ]
  ...
  const EmptyComponent = () => (
    <View style={tw`items-center justify-center py-8 px-4`}>
      ...
      <Text
        style={[
          styles.globalText,
          tw`text-lg text-gray-800 mb-2 text-center font-medium`,
        ]}
      >
  ...
  const flatData = useMemo(
    () => paymentMethods.filter((item) => item.is_active),
    [paymentMethods]
  );
