...
export const editAddress = (req, res) => {
  ...
  con.query(sql, values, (err, result) => {
    ...
    if (result.affectedRows === 0) {
      // If no rows were affected (meaning the address_id didn't match any record)
  ...
  con.query(sql, values, (err, result) => {
    if (err) {
      if (err.code === 'ER_NO_SUCH_TABLE') {
        // Example of handling when the table is not found
...
export const deleteBookmark = (req, res) => {
  ...
  const values = [req.params.address_id]; // รับค่าจาก URL แทน body
  ...
  con.query(sql, values, (err, result) => {
    ...
    return res.status(200).json({
      Status: true,
      Message: "ที่อยู่ถูกลบเรียบร้อยแล้ว",
      AffectedRows: result.affectedRows,
    });
  });
};
...
`;
  ...
  con.query(sql, values, (err, result) => {
      if (err) {
          return res.status(500).json({ Status: false, Error: err.message , Message: "เกิดข้อผิดพลาดในการลบที่อยู่" });
      }

      if (result.affectedRows === 0) {
          return res.status(404).json({ Status: false, Message: "ไม่พบที่อยู่ที่ต้องการลบ"  });
      }
...
export const getuserBookmarks = (req, res) => {
  ...
  con.query(sql, [customer_id], (err, result) => {
    if (err) {
      return res
        .status(500)  // ใช้ 500 Internal Server Error หากเกิดข้อผิดพลาดที่เซิร์ฟเวอร์
        .json({ Status: false, Error: 'เกิดข้อผิดพลาดที่เซิร์ฟเวอร์: ' + err.message });
    }
    
    if (result.length === 0) {
      return res
        .status(404)  // ใช้ 404 Not Found หากไม่พบข้อมูล
        .json({ Status: false, Error: "ไม่พบข้อมูล Bookmark" });
    }
    
    return res
      .status(200)  // ใช้ 200 OK หากคำขอสำเร็จ
